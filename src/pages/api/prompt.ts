...
const summarizeLargeText = async (text: string) => {
  ...
  for (const chunk of chunks) {
    ...
    //   summaries.push(summarize(chunk))
  }
  console.log('je;;p', summaries)
  return summaries.join(' ')
  // return results(await Promise.all(summaries))?.join(' ')
  //   console.log(chunks)
  //   const promises = chunks.map((chunk) => summarize(chunk))
  //   const summaries = (await Promise.all(promises)).join(' ')
  //   return summaries
}
...
const handler = async (req: NextRequest, res: NextResponse) => {
  ...
  const tex = `Heart disease refers to any condition affecting the heart. There are many types, some of which are preventable.
  ...
  In this article, learn more about the types, causes, and symptoms of heart disease. This article also covers risk factors and treatment. Types
  ...
  const largeText = await summarizeLargeText(tex)
